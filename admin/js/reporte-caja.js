...
function dibuja_tabla_caja(datos){
	...
	for (var i = datos.length - 1; i >= 0; i--) {
		ayudante_ventas += parseFloat( datos[i].venta );
		...
		.append( 
			...
			.append(
				...
				$("<td>").html( (datos[i].venta == 0) ? "No aplica" : "$" + datos[i].venta ),
				$("<td>").html( (datos[i].gastos == 0) ? "No aplica" : "$" + datos[i].gastos ),
				$("<td>").html(datos[i].fch_venta),
	...
	$("#caja_chica").text(formatoMexico(ayudante_caja_chica.toFixed(2))).parent().show();
	$("#gastos").text(formatoMexico(ayudante_gastos.toFixed(2))).parent().show();
	$("#ventas").text(formatoMexico((ayudante_ventas.toFixed(2)))).parent().show();
	$("#total_caja").text(formatoMexico(ayudante_total.toFixed(2))).parent().show();
	$("#utilidad").text(formatoMexico(ayudante_utilidad.toFixed(2))).parent().show();
...
function consulta_caja_fecha(fecha_inicio, fecha_fin){
	$.post('../admin/modulos/caja/consultar_caja_fecha.php', {fecha_inicio: fecha_inicio, fecha_fin: fecha_fin}, function(respuesta) {
