...
function Venta(no_venta, fch_venta, nombre_productos, numero_productos, total, id_usuario, utilidad) {
    this.no_venta = no_venta;
    this.fch_venta = fch_venta;
    this.lista_productos = [];
    this.numero_productos = parseFloat(numero_productos);
    this.total = total;
    this.lista_productos.push(new Producto(numero_productos, nombre_productos));
    this.id_usuario = id_usuario;
    // this.familia = familia;
    this.utilidad = utilidad;
}

Venta.prototype.agrega_producto_lista = function (numero_productos, nombre_productos) {
    this.numero_productos += parseFloat(numero_productos);
    this.lista_productos.push(new Producto(numero_productos, nombre_productos));
};
...
//     $.get("../admin/modulos/familias/consultar_familias.php", function (familias) {
...
function consulta_ventas_fecha(fecha_inicio, fecha_fin, familia) {
    $.post('../admin/modulos/ventas/consultar_todas_las_ventas.php', {
...
//     });
// }
function dame_posicion_venta(ventas, no_venta) {
    for (var i = ventas.length - 1; i >= 0; i--) {
        if (ventas[i].no_venta === no_venta) return i;
    }
    return -1;
}
...
function dibuja_tabla_ventas(ventas) {
    ...
    for (var i = ventas.length - 1; i >= 0; i--) {
        ...
        if (ayudante_numero_venta === ventas[i].no_venta) {
            var posicion = dame_posicion_venta(no_venta, ventas[i].no_venta);
            no_venta[posicion].agrega_producto_lista(ventas[i].numero_productos, ventas[i].nombre_productos);
            no_venta[posicion].total = parseFloat(no_venta[posicion].total) + parseFloat(subtotal);
            no_venta[posicion].utilidad = parseFloat(no_venta[posicion].utilidad) + parseFloat(subtotal_utilidad);
            ...
            ayudante_numero_venta = ventas[i].no_venta;
        }
    }
    for (var i = no_venta.length - 1; i >= 0; i--) {
        ayudante_total += parseFloat(no_venta[i].total);
        ayudante_utilidad += parseFloat(no_venta[i].utilidad);
        $("#contenedor_tabla tbody")
            .append(
                $("<tr>")
                    .append(
                        $("<td>").html(no_venta[i].no_venta),
                        $("<td>").html(no_venta[i].fch_venta),
                        $("<td>").html(no_venta[i].productos_como_html()),
                        $("<td>").html(no_venta[i].numero_productos),
                        $("<td>").html("$" + formatoMexico(no_venta[i].total)),
                        $("<td>").html("$" + formatoMexico(no_venta[i].utilidad)),
                        $("<td>").html(no_venta[i].id_usuario)
                    )
            );
    }
    $("#contenedor_tabla").animateCss("fadeInUp");
    $("#mostrar_total").text(formatoMexico(ayudante_total.toFixed(2))).parent().show();
    $("#mostrar_utilidad").text(formatoMexico(ayudante_utilidad.toFixed(2))).parent().show();
