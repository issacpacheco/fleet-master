...
function Producto(id, sku, nombre, cantidad, precio, posicion, precio_original) {
    this.id = id;
    this.sku = sku;
    this.nombre = nombre;
    ...
    this.precio_original = parseFloat(precio_original);
    this.utilidad = (this.precio_venta - this.precio_original >= 0 ? this.precio_venta - this.precio_original : 0 );
};
...
function agrega_producto_local(producto) {
    var ya_esta_en_la_lista = producto_ya_esta_en_lista(producto.sku);
...
function producto_ya_esta_en_lista(codigo) {
    for (var x = 0; x < productos_vender.length; x++) {
        if (productos_vender[x].sku === codigo) {
...
function realizar_venta(productos, total, cambio, ticket) {
    ...
    $.post('../admin/modulos/ventas/realizar_venta.php', {
        "productos": productos,
        "total": total,
        "ticket": ticket,
        "cambio": cambio
...
function autocompletado_input() {
    var opciones = {
        ...
        url: function (busqueda) {
            var sugerencias = "../admin/modulos/ventas/autocompletado.php?busqueda=" + busqueda;
            return sugerencias;
        },
        ...
        list: {
            ...
            onChooseEvent: function () {
                ...
                comprueba_si_existe_codigo(producto_seleccionado.sku);
            },
...
function comprueba_si_existe_codigo(codigo) {
    $.post('../admin/modulos/ventas/comprueba_si_existe_codigo.php', {"codigo": codigo}, function (respuesta) {
        $("#codigo_producto")
            .val("")
            .trigger(
                jQuery.Event(
