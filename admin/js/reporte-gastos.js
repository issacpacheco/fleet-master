	...
	for (var i = datos.length - 1; i >= 0; i--) {
		...
		.append( 
			...
			.append(
				...
				$("<td>").html(datos[i].fch_gasto),
				$("<td>").html(datos[i].usuario)
			)
		);
	}
	ayudante_total = Math.round(ayudante_total * 100) / 100;
	$("#total_gastos").text(formatoMexico(ayudante_total.toFixed(2))).parent().show();
	$( "#generar_reporte" ).show();
	return;
}

function consulta_gastos(fecha_inicio, fecha_fin){
	$.post('../admin/modulos/gastos/consultar_todos_los_gastos.php', {fecha_inicio: fecha_inicio, fecha_fin: fecha_fin}, function(respuesta) {
